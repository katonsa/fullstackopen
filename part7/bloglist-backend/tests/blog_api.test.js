...
beforeAll(async () => {
  ...
  usersForTests = await Promise.all(
...
beforeEach(async () => {
  ...
  const blogsToSave = initialBlogs.map(
    (blog) =>
      new Blog({
        ...blog,
        user: user.id,
...
describe('addition of a blog(post)', () => {
  ...
  test('property likes have default value of 0 when missing from the request', async () => {
    ...
    const newlyAddedBlog = blogsAfter.find(
...
describe('updating a blog(post)', () => {
  test('succeeds with valid data and token', async () => {
    ...
    const updatedItem = itemsAfterChanges.find(
      (item) => item.title === itemToUpdate.title,
    );
